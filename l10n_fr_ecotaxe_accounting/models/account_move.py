...
class AccountMove(models.Model):
    ...
    ecotaxe_move_id = fields.Many2one("account.move", readonly=True, copy=False)

    def _create_prepare_ecotax_move_vals(self):
        self.ensure_one()
        ecotax_journal_id = self.company_id.ecotaxe_journal_id.id
    ...
    def _manage_ecotax_isolation(self):
        self.ensure_one()
        ecotax_lines = self.invoice_line_ids.ecotaxe_line_ids
        account_ecotax_mapping = defaultdict(float)
        product_account_mapping = defaultdict(float)
        for ecotax_line in ecotax_lines:
            account = (
                ecotax_line.ecotaxe_classification_id.ecotaxe_account_id
                or self.company_id.ecotaxe_account_id
            )
            if not account:
                continue
            account_ecotax_mapping[account] += ecotax_line.ecotaxe_amount_total
            product_account_mapping[
                ecotax_line.account_move_line_id.account_id
            ] += ecotax_line.ecotaxe_amount_total
        if not account_ecotax_mapping:
            return self.env["account.move"]
        ecotax_move = self.ecotaxe_move_id
        if ecotax_move:
            if ecotax_move.state != "cancel":
                raise exceptions.ValidationError(
                    _("The linked ecotaxe entry should be canceled.")
                )
            ...
            self.write({"ecotaxe_move_id": ecotax_move.id})
        line_vals_list = []
